...
typedef struct {
    ...
    uint16_t seq_num:12;
} __attribute__((packed)) wifi_seq_ctrl_t;
...
} __attribute__((packed)) wifi_beacon_body_t;
...
typedef struct {
    ...
    // -- body --
    uint16_t reason; // 
} __attribute__((packed)) wifi_deauth_deaus_body_t;
...
typedef struct {
    ...
    wifi_seq_ctrl_t seq_ctrl; 
    uint8_t body[0]; 
} __attribute__((packed)) wifi_common_body_t;

typedef struct {
    uint8_t id;
    uint8_t len;
    uint8_t data[0];
} __attribute__((packed)) wifi_var_item_t;
